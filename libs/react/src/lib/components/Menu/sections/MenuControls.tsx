...
export const MenuControls = forwardRef<HTMLDivElement>(function (_props, ref) {
  ...
  return (
    <section
      ref={ref}
      className={clsx('hv-menu-controls', menuPosition === 'bottom' ? 'order-3' : '-order-1')}
    >
      <button className="hv-menu-controls-button" onClick={() => toggleMenuVisibility()}>
        <MenuIcon size={15} />
        <span>{isMenuVisible ? 'Hide' : 'Menu'}</span>
      </button>

      <button className="hv-menu-controls-button" onClick={() => toggleCameraVisibility()}>
